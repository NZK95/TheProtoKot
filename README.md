...
- [ ] Добавить премку, которая будет расширять лимиты к созданию подписей и документов, автообновление данных всех серверов и обновление данных без задержки, проверка чёрного списка по всем серверам, расширить лимиты аккаунтов в чекере жалоб, эксклюзивные подсказки и AHK1.
### Выполнено
...
> [!WARNING]
> Важно, что бы подпись пользователя имела прозрачный фон, потому что она накладывается сверху штампа.

- **🚫 Чёрный список** - Система проверки игроков по базам чёрных списков различных серверов. Пользователь вводит никнейм и сервер, после чего бот выполняет поиск по Excel таблице чёрного списка конкретного сервера, и отображает подробную информацию о блокировке.
> [!NOTE]
> Excel таблицы обновляются каждые 30 секунд.
...
- **🧑‍💻Админ панель** - Удобная панель которая позволяет отслеживать активность пользователей (например количество пользователей за 5 минут или за 1 день), взаимодействовать с базами данных, контроллировать лимиты в чекере жалоб, банить или кикать подьзователя, делать рассылку. Пока что доступна только *chatIds*, которые находятся в списке Admins в модуле TelegramConstants.cs. Планируется перенести список админов в *appsettings.json* для более удобного взаимодействия.
> [!NOTE]
